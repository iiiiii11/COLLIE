...
def get_exp_name(args: argparse.Namespace, g_start_time: int, exp_suffix=''):
    exp_name = ''
    ...
    if exp_suffix:
        exp_name += '_' + exp_suffix

    return exp_name, exp_name_prefix


def get_log_dir(args: argparse.Namespace, g_start_time: str, EXP_DIR: str, exp_suffix='') -> Any:
    """Get logging directory.

    Args:
        args (argparse.Namespace): argparse arguments
        g_start_time (str): time in format %Y%m%d-%H%M%S
        EXP_DIR (str): experiment directory

    Returns:
        Any: logging directory
    """
    exp_name, exp_name_prefix = get_exp_name(args, g_start_time, exp_suffix)
    assert len(exp_name) <= os.pathconf('/', 'PC_NAME_MAX')
    date_str = g_start_time[:6]
    # Resolve symlinks to prevent runs from crashing in case of home nfs crashing.
    log_dir = os.path.realpath(os.path.join(EXP_DIR, args.run_group, date_str, exp_name))
    ...
    if normalizer_type == 'off':
        ...
        normalizer_mean = np.array(
            [0.00486117, 0.011312, 0.7022248, 0.8454677, -0.00102548, -0.00300276, 0.00311523, -0.00139029,
             0.8607109, -0.00185301, -0.8556998, 0.00343217, -0.8585605, -0.00109082, 0.8558013, 0.00278213,
             0.00618173, -0.02584622, -0.00599026, -0.00379596, 0.00526138, -0.0059213, 0.27686235, 0.00512205,
             -0.27617684, -0.0033233, -0.2766923, 0.00268359, 0.27756855, 0.00486117, 0.011312, 0.7022248, 0.8454677, -
             0.00102548, -0.00300276, 0.00311523, -0.00139029,
             0.8607109, -0.00185301, -0.8556998, 0.00343217, -0.8585605, -0.00109082, 0.8558013, 0.00278213,
             0.00618173, -0.02584622, -0.00599026, -0.00379596, 0.00526138, -0.0059213, 0.27686235, 0.00512205,
             -0.27617684, -0.0033233, -0.2766923, 0.00268359, 0.27756855])
        normalizer_std = np.array(
...
def record_video(runner, label, trajectories, n_cols=None, skip_frames=1):
    ...
    for trajectory in trajectories:
        if 'render' not in trajectory['env_infos']:
            continue
